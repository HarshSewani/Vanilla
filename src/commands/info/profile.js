...
class Badges extends VanillaCommand{
    get name(){
        return 'profile';
    }
    get aliases(){
    ...
    async run(client,message,args,prefix)
    {
        try{
            ...
            let guild = await client.guilds.fetch('1004061467668123689');
            user = await guild.members.fetch(member.id).catch((e) =>{ badges += `\`No Badges Available\` <a:ares_badges:1103681708605317140> \n You must be avaiable in our [support server](https://discord.gg/bHrDKJDSYC) to get your badges\nConsider Joining Support server by clicking [here](https://discord.gg/bHrDKJDSYC)`;}) 
            ...
            try{
                ...
                if(badges === '') badges += `\n ${client.emoji.users} **User**`; }
                    ...
                    badges = `\`No Badges Available\` <a:badges:1109726177528119336> \n You must be avaiable in our [support server](${client.config.server}) to get your badges\nConsider Joining Support server by clicking [here](${client.config.server})`;
                }
            
            return message.channel.send({embeds : [new EmbedBuilder().setColor(client.config.color).setAuthor({name : `Profile for ${member.tag}`}).addFields({name : `__BADGES__ <a:badges:1109726177528119336>` , value : `${badges}`}).setThumbnail(member.displayAvatarURL({dynamic : true}))]})
        } catch(e) { 
            console.log(e)
            let badges = '';
            badges = `\`No Badges Available\` <a:badges:1109726177528119336> \n You must be avaiable in our [support server](${client.config.server}) to get your badges\nConsider Joining Support server by clicking [here](${client.config.server})`;
...
module.exports = Badges;
    ...
    //         if(badges === '') badges += `\n ${client.emoji.users} **User**`; }
    //         catch(e) { 
    //             badges = `\`No Badges Available\` <a:ares_badges:1103681708605317140> \n You must be avaiable in our [support server](${client.config.server}) to get your badges\nConsider Joining Support server by clicking [here](${client.config.server})`;
    //         }
        
    //     return message.channel.send({embeds : [new EmbedBuilder().setColor(client.config.color).setAuthor({name : `Profile for ${member.tag}`}).addFields({name : `__BADGES__ <a:ares_badges:1103681708605317140>` , value : `${badges}`}).setThumbnail(member.displayAvatarURL({dynamic : true}))]})
    // } catch(e) { 
    //     console.log(e)
    //     let badges = '';
...
// module.exports = Badges;
// return message.channel.send({embeds : [new EmbedBuilder().setColor(client.config.color).setDescription(`**This command is disabled by the owner!** <@948980125251297323> <a:CF_ownership:1103675705306382467>`).setThumbnail(message.author.displayAvatarURL({dynamic : true}))]})