...
class Queue extends VanillaCommand{
    get name(){
        return 'queue';
    }
    get aliases(){
    ...
    async run(client,message,args,prefix,player){
        ...
        {
            let queuedSongs = player.queue.map((track,index) => `\`[${index + 1}]\` [${track.title.substring(0,45)}](${client.config.server}) ${client.emoji.arrow} ${ms(track.length)}`);
