...
class Play extends VanillaCommand{
    get name(){
        return 'play'
    }
    get aliases(){
        ...
        try{
            ...
            if(query.startsWith('https://'))
            {
                ...
                if(query.includes(`spotify`))
                {
                    ...
                    let node = client.lavasfy.nodes.get("H4RRY");
            ...
            let emb = new EmbedBuilder().setColor(client.config.color).setAuthor({name : `Select From Where To Play`,iconURL : message.author.displayAvatarURL({dynamic : true})}).setDescription(`__**NOTE**__: \`If You select Spotify AutoPlay Will Not Work Choose Default To Use Autoplay\``);
            
            let b1 = new ButtonBuilder().setStyle(ButtonStyle.Secondary).setEmoji(`<:icons8dj641:1155153076651577364>`).setLabel(`Default`).setCustomId(`def`);
            let b2 = new ButtonBuilder().setStyle(ButtonStyle.Secondary).setEmoji(`<:icons8spotify64:1155378607863111680>`).setLabel(`Spotify`).setCustomId(`spoti`);
